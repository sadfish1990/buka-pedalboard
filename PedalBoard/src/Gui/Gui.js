...
export default class pedalboardGui extends HTMLElement {
  ...
  async init() {
    ...
    this.clearBtn.innerHTML = "🗑️ Clear All";
    this.clearBtn.style.padding = "5px 10px";
    this.clearBtn.style.backgroundColor = "#d32f2f";
    this.clearBtn.style.color = "white";
    ...
    fullscreenBtn.style.backgroundColor = "#1976d2";
    ...
    fullscreenBtn.addEventListener("click", () => {
      if (!document.fullscreenElement) {
        this._root.host.requestFullscreen().catch(err => {
    ...
    this.main.appendChild(buttonsContainer);
    await this.loadThumbnails();
    this.createBoard();

    // Setup Clear All listener after board is created (Chrome compatibility)
    this.clearBtn.addEventListener("click", () => {
      const pedalCount = this.board.childNodes.length;
      if (pedalCount === 0) {
        alert("La pedalera ya está vacía");
        return;
      }
      if (confirm(`¿Eliminar todos los ${pedalCount} pedales?\n\nEsta acción no se puede deshacer.`)) {
    ...
    return new Promise((resolve, reject) => {
      new ResizeObserver(function (entries) {
        // Resolve on any valid width (Responsive Fix)
   ...
   */
  setStyle() {
    this.style.display = "block"; // Changed to block for full size
