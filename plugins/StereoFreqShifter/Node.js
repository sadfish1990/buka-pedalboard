    ...
    ctrlChange(channel, ctrl, value) {
        if (this.fCtrlLabel[ctrl] !== []) {
            for (var i = 0; i < this.fCtrlLabel[ctrl].length; i++) {
                ...
                this.setParamValue(path, StereoFreqShifterNode.remap(value, 0, 127, this.fCtrlLabel[ctrl][i].min, this.fCtrlLabel[ctrl][i].max));
                if (this.output_handler) {
        ...
        try {
            ...
            if (this.fWorkletProcessors.indexOf(name) === -1) {
                try {
                    ...
                    let real_url = window.URL.createObjectURL(new Blob([StereoFreqShifterProcessorString1], { type: 'text/javascript' }));
            ...
            this.node.onprocessorerror = () => { console.log('An error from StereoFreqShifter-processor was detected.'); }
            return (this.node);
        } catch (e) {
            console.error(e);
            console.error("Faust " + this.name + " cannot be loaded or compiled");
